...
export default function CartPage() {
  ...
  if (isError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center">
        ...
        <p className="text-gray-500 mb-6">You need to login to view your cart.</p>
        <Link href="/login" className="bg-primary text-white px-6 py-2 rounded-full">Login Now</Link>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center pt-32 text-center bg-background">
        ...
        <Link href="/shop" className="bg-primary text-white px-8 py-3 rounded-full font-bold">Start Shopping</Link>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center mb-4">
            <span className="font-medium text-text-muted">{items.length} Items</span>
          ...
          {items.map((item) => (
            <div key={item.cartItemId} className="flex gap-4 bg-card p-4 rounded-2xl border border-border/50 shadow-sm">
              {/* Image */}
              ...
              <div className="flex flex-col grow justify-between">
                <div className="flex justify-between items-start">
                  <Link href={`/products/${item.productId}`} className="font-bold text-lg hover:text-primary line-clamp-1">
                    {item.name}
                  </Link>
                  <span className="font-bold text-lg">Rs. {item.itemTotal.toLocaleString()}</span>
                </div>
                ...
                <div className="flex justify-between items-end mt-4">
                  ...
                  <div className="flex items-center gap-3 bg-background border border-border/50 rounded-full px-3 py-1">
                    ...
                    <button 
                      ...
                      className="text-gray-500 hover:text-primary transition disabled:opacity-30"
                    >
                      <Plus size={16} />
                    </button>
                  </div>
                  ...
                  <button 
                    onClick={() => updateItem({ productId: item.productId, quantity: 0 })}
        ...
        {/* Right: Order Summary */}
        <div className="bg-card p-6 rounded-3xl border border-border/50 shadow-sm h-fit sticky top-28">
          <h2 className="text-xl font-bold mb-6 border-b border-border/50 pb-4">Order Summary</h2>
          
          <div className="space-y-4 mb-6">
            <div className="flex justify-between text-text-muted">
              <span>Subtotal</span>
              <span className="font-medium text-text-main">Rs. {subtotal.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-text-muted">
              <span>Shipping</span>
