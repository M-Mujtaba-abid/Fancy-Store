...
import {
  ...
  processCheckoutCompletedService,
...
export const createCheckoutSession = async (req, res) => {
  try {
    const { amount, currency, productName, userId, shippingInfo, cartItems } = req.body;
    ...
    const url = await createCheckoutSessionService({
      amount, currency, productName, userId, shippingInfo, cartItems,
    });
...
export const stripeWebhook = async (req, res) => {
  ...
  try {
    event = handleWebhookService(req.body, sig);
  } catch (err) {
    console.error("❌ Webhook signature verification failed:", err.message);
    return res.status(400).send(`Webhook Error: ${err.message}`);
  }
  ...
  if (event.type === "checkout.session.completed") {
    try {
      await processCheckoutCompletedService(event.data.object);
    } catch (err) {
